...
- **Feina realitzada en aquesta sessió**:
  - [x] **🔋 Mode d'Estalvi de Bateria & Renderitzat Intel·ligent (`BatteryRenderPolicy`)**:
    ...
    - Filtre de rumb amb supressió de jitter de compàs ($< 4^\circ$) i desactivació d'efectes pesats de GPU `backdrop-filter: blur()` a tot el CSS de l'aplicació.
    - Botons i indicadors dedicats `#btn-eco-toggle` i `#btn-top-eco`.
  ...
  - [x] **🧪 Suite Ampliada a 36 Proves Unitàries Automatitzades (`npm test`)**:
    - 36 tests passant al 100% verificant geometria, altimetria, GPX, persistència IndexedDB, descàrrega per URL, scrubbing, validació de backups, waypoints a Turn-by-Turn i polítiques de bateria.
  - [x] **Sincronització de Codi i Compilació**:
    - Còpies PWA `index.html` i `web-app/index.html` amb hash SHA256 idèntic (`db90b74c...`).
    - Compilació TypeScript Expo (`npx tsc --noEmit`) 100% neta.

- **Punt exacte on ens hem quedat**:
  - Fase 0 completada al 100%.
  - Fase 1 i Fase 2 consolidades amb biblioteca IndexedDB, mode d'estalvi de bateria i renderitzat intel·ligent, editor de waypoints, backups JSON a la UI, descàrrega per URL, perfil interactiu ClimbPro i 36 tests unitaris.
  - Preparat per continuar amb les proves de camp o la integració a React Native / Expo.
...
- **Tasques immediates per reprendre**:
  ...
  - [ ] **🗺️ Preparar col·lecció de GPX de prova reals**:
    - Proves amb tracks circulars de més de 50 km, múltiples segments i desnivell sever.
